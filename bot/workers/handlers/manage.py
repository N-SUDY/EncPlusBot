...
async def allowautoencode(event, args, client):
    """
    Turns auto-encode (encode on file receive) on/off.
    Seeded from AUTO_ENCODE env var, but toggleable here at runtime.

    Required arguments:
        - off/disable (disable auto-encode)
        - on/enable (enable auto-encode)
        <All arguments are not case sensitive.>
    """
    if not user_is_owner(event.sender_id):
        return await event.delete()
    autoencode = get_var("autoencode")

    if not args:
        if autoencode:
            return await event.reply("`Auto-Encode is enabled.`")
        else:
            return await event.reply("`Auto-Encode is disabled.`")

    if args.casefold() == "off" or args.casefold() == "disable":
        if not autoencode:
            return await event.reply("**Already turned off**")
        autoencode.clear()
        await event.reply("**Auto-Encode Turned off Successfully**")
    if args.casefold() == "on" or args.casefold() == "enable":
        if autoencode:
            return await event.reply("**Already turned on**")
        autoencode.append(1)
        await event.reply(
            "**Auto-Encode Turned on Successfully**\n__Persists till bot restarts!__"
        )


async def allowautothumb(event, args, client):
    """
    Turns auto-thumbnail (auto-generate thumbnail from video frame on
    upload when no custom thumbnail is set) on/off.

    Required arguments:
        - off/disable (disable auto-thumbnail)
        - on/enable (enable auto-thumbnail)
        <All arguments are not case sensitive.>
    """
    if not user_is_owner(event.sender_id):
        return await event.delete()
    autothumb = get_var("autothumb")

    if not args:
        if autothumb:
            return await event.reply("`Auto-Thumbnail is enabled.`")
        else:
            return await event.reply("`Auto-Thumbnail is disabled.`")

    if args.casefold() == "off" or args.casefold() == "disable":
        if not autothumb:
            return await event.reply("**Already turned off**")
        autothumb.clear()
        await event.reply("**Auto-Thumbnail Turned off Successfully**")
    if args.casefold() == "on" or args.casefold() == "enable":
        if autothumb:
            return await event.reply("**Already turned on**")
        autothumb.append(1)
        await event.reply(
            "**Auto-Thumbnail Turned on Successfully**\n__Persists till bot restarts!__"
        )
...
async def del_thumb(event, args, client):
    """Delete saved default thumbnail"""
    if not user_is_owner(event.sender_id):
        return await event.delete()
    if not file_exists(thumb):
        return await event.reply("**No custom thumbnail set.**")
    s_remove(thumb)
