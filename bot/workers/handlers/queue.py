...
async def listqueue(event, args, client, deletable=True):
    ...
    # queue[0] is the currently-encoding item; pendng items start at index 1
...
async def enleech(event, args: str, client, direct=False):
    ...
    no_dl_spt_msg = "`File to download is…\neither not a video\nor is a batch torrent.\nUse /lb to leech batch torrents.`"
    ...
    try:
                ...
                async with queue_lock:
                    for _none, _id in zip(
                        ...
                        if not is_video_file(file.name):
                            # could be a batch torrent folder — allow it through
                            pass
        ...
        is_batch = not is_video_file(file.name)
        if is_batch:
            return await event.reply(
        ...
        if len(queue) > 1 or bot_is_paused():
            msg = await event.reply(
                f"**Torrent added To Queue ⏰, POS:** `{pos}`\n`Please Wait , Encode will start soon`"
            )
            if len(queue) > 1:
...
async def enlb(event, args: str, client, direct=False):
    """
    /lb — Batch leech command.
    Accepts a magnet link or .torrent URL pointing to a batch (multi-file) torrent.
    Resolves the file list, populates bqueue, then adds to encode queue.
    Supports the same flags as /leech (-f, -rm, -n, -v, -da, etc.)
    """
    import re as _re
    chat_id = event.chat_id
    user_id = event.sender_id
    ...
    try:
        ...
        if not uri:
            return await event.reply("`Send /lb with a magnet link or torrent URL.`")
        if not (is_url(uri) or is_magnet(uri)):
            return await event.reply("`Invalid magnet/torrent link.`")
        ...
        if is_video_file(file.name):
            return await event.reply(
                "`This looks like a single-file torrent.`\nUse `/l` or `/leech` instead."
            )

        for item in queue.values():
            if file.name in item:
                return await event.reply("`This torrent is already in queue.`")

        from bot.utils.bot_utils import Qbit_c, get_aria2, sync_to_async
        from bot.workers.downloaders.dl_helpers import get_torrent_files

        _resolving_msg = await event.reply("`Resolving torrent metadata…`")
        try:
            aria2 = get_aria2()
            if not aria2:
                raise ValueError("aria2 is not available.")
            dl, flist, _name = await get_torrent_files(
                uri, aria2, dir_path="downloads", action="remove"
            )
            if not flist:
                raise ValueError("No resolvable files (metadata timeout or empty torrent).")

            def _natural_key(p):
                base = os.path.basename(p)
                return [int(c) if c.isdigit() else c.lower() for c in _re.split(r"(\d+)", base)]
            flist = sorted(flist, key=_natural_key)

            torrent = Qbit_c(count=len(flist), flist=flist)
            torrent.name = file.name
            batch_q = {i: (1 if is_video_file(os.path.basename(f)) else 3) for i, f in enumerate(flist)}
            video_count = sum(1 for v in batch_q.values() if v == 1)
        except Exception:
            await logger(Exception)
            await _resolving_msg.edit("`Failed to resolve torrent metadata. Try again.`")
            return
        await _resolving_msg.delete()
        ...
        async with queue_lock:
            ...
            bqueue = get_bqueue()
            bqueue[(chat_id, event.id)] = [torrent, batch_q]
        ...
        file_list_str = "\n".join(
            ...
            if is_video_file(os.path.basename(f))
        )
        header = (
            f"**Batch torrent queued ⏰, POS:** `{pos}`\n"
            f"**{video_count} video files will be encoded:**\n"
        )
        pre_event = event
        for smsg in await split_text(header + file_list_str):
    ...
    try:
        ...
        for item in queue.values():
            if name in item:
                ...
                return await message.reply("**THIS FILE HAS ALREADY BEEN ADDED TO QUEUE**", quote=True)
        anilist = True
        cust_fil = cust_v = str()
        force_name = None
        uri = None
...
async def addqueue(event, args, client):
    ...
    try:
        ...
        if not (command and check_cmds(command, "/l", "/leech")):
