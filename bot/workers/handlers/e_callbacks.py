...
async def pres(e):
    try:
        ...
        req_info = decode(_id)
        if not req_info:
            return await e.answer("⚠️ Bot was restarted. Encoding info unavailable.", alert=True)
...
async def skip(e, skip_jobs=False):
    ...
    req_info = decode(_id)
    if not req_info:
        return await e.answer("⚠️ Bot was restarted. Use /clean to cancel current job.", alert=True)
...
async def stats(e):
    try:
        ...
        req_info = decode(_id)
        if not req_info:
            return await e.answer("⚠️ Bot was restarted. Stats unavailable.", alert=True)
        process, dl, out, user_id, stime = req_info
...
async def download_button_callback(*args):
    callback_query = args[-1]
    try:
