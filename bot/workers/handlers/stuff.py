...
async def start(event, args, client):
    ...
    await event.reply(
        ...
        buttons=[
            ...
            [
                Button.url("Source Code", url="github.com/TheMisfitDK/EncPlusBot"),
                Button.url("Original: Danish_00", url="github.com/1Danish-00/compressorqueue"),
            ],
            [
                Button.url("Original: Nubuki", url="github.com/Nubuki-all/Enc"),
                Button.url("Owner", url="t.me/I_am_Ryan_Gosling"),
            ],
        ],
    )
...
async def ihelp(event):
    await event.edit(
        ...
        "Encode videos using your custom FFmpeg/HandBrake settings.\n\n"
        "📌 **Usage:**\n"
        "• Reply to a video with `/add` to queue it\n"
        "• Use `/leech <url>` or `/l <url>` for direct links, magnets & torrents\n"
        "• Use `/set` to change FFmpeg params\n"
        "• Use `/get` to view current FFmpeg params\n\n"
        "🧲 **Magnet/torrent links** are handled by aria2 — just paste into `/leech`.\n\n"
        "⚠️ Auto-encode on file send is **disabled** — use `/add` instead.\n\n"
        "Click Commands for full list.",
        buttons=[
...
async def temp_unauth(event, args, client):
    ...
    if not user_is_owner(sender):
        return await event.reply("Not Happening.")
    if event.is_reply:
...
async def temp_auth(event, args, client):
    ...
    if not user_is_owner(sender):
        return await event.reply("Nope, not happening.")
    if event.is_reply:
        rep_event = await event.get_reply_message()
        new_id = rep_event.sender_id
    else:
...
async def icommands(event):
    ...
    await event.edit(
        f"""📋 **Commands** (suffix: `{s or 'none'}`)

🎬 **Encoding**
`add{s}` — queue a replied video
`l{s}` / `leech{s}` — download & queue a URL, magnet or .torrent
`s{s}` — select file index from a batch torrent
`queue{s}` — view queue
`batch{s}` — preview batches
`list{s}` — show torrent/magnet name via aria2
`clear{s}` — clear queue
`cancelall{s}` — cancel all + cleanup
`pause{s}` — pause/resume bot

⚙️ **FFmpeg Settings**
`get{s}` — view current FFmpeg cmd
`set{s}` — set custom FFmpeg cmd
`reset{s}` — reset to default FFmpeg
`mget{s}` — view mux args
`mset{s}` — set/reset mux args
`mux{s}` — remux a replied file

📁 **Files & Media**
`download{s}` — download file/link to bot
`upload{s}` — upload local file/link
`rename{s}` — rename & re-upload video
`m{s}` — mediainfo of replied file/link
`showthumb{s}` — show current thumbnail

🎌 **Filters & Naming**
`filter{s}` — manage filters
`vfilter{s}` — view filters
`delfilter{s}` — delete filter
`name{s}` — quick anime name filter
`vname{s}` — view name filters
`delname{s}` — delete name filter
`setrename{s}` — set rename format
`parse{s}` — toggle Anilist/caption parsing
`airing{s}` — anime airing schedule
`anime{s}` — anime info from Anilist

📡 **RSS**
`rss{s}` — manage RSS feeds

🔧 **Admin / Owner**
`start{s}` / `help{s}` — bot info
`ping{s}` — check bot latency
`status{s}` — server & bot stats
`permit{s}` — add temp user
`unpermit{s}` — remove temp user
`groupenc{s}` — toggle group encoding
`forward{s}` — forward to fchannel
`v{s}` — versioning toggle
`restart{s}` — restart bot
`update{s}` — pull latest update
`nuke{s}` — ☢️ stop bot
`bash{s}` — run shell command
`eval{s}` — evaluate Python code
`peval{s}` — eval (Pyrogram client)
`logs{s}` — get bot logs

🧲 **Downloads via aria2** (magnets, torrents, direct links)
All handled by `/leech` — no separate torrent command needed.

💡 All commands accept `-h` / `--help` for details.""",
