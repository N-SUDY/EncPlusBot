    ...
    Just pass any of the following:
        ...
        -
            ...
            -qs (optional argument)
                <int>: selects file index to use from batch torrent (via aria2)
    __as an argument.
    """
    if not user_is_owner(event.sender_id):
        return await event.delete()
...
async def en_list(event, args, client):
    """
    List files inside a torrent/magnet link via aria2.
    Arguments:
    ...
    try:
        ...
        from bot.utils.bot_utils import get_aria2
        from bot.workers.downloaders.dl_helpers import get_torrent_files

        aria2 = get_aria2()
        if not aria2:
            return await event.reply("`E404: Aria2 is not available.`")

        status_msg = await event.reply("`Fetching file list…`")

        dl, raw_flist, name = await get_torrent_files(
            args, aria2, dir_path="temp/list_probe", action="remove"
        )

        flist = [str(f).replace(f"temp/list_probe/", "").replace(f"temp/list_probe", "") for f in raw_flist]

        if not flist:
            msg = f"**{name}**\n\n`No files found.`"
        elif len(flist) == 1:
            msg = f"**{name}**\n\n`0.` `{os.path.basename(flist[0])}`"
        else:
            lines = [f"**{name}**\n"]
            for i, f in enumerate(flist):
                lines.append(f"`{i}.` `{os.path.basename(f)}`")
            lines.append(f"\n**Total:** `{len(flist)} files`")
            msg = "\n".join(lines)
