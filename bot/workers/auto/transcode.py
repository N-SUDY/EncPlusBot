...
from bot import asyncio, mux_file, pyro, queue_lock, tele, time
...
from bot.utils.batch_utils import (
    batch_dl_should_retry,
    clear_batch_dl_retry,
    get_batch_list,
...
from bot.workers.downloaders.download import release_batch_torrent
...
async def something():
    while True:
        await thing()
        # ponytail: yield to event loop between iterations to prevent hot-spinning
        # when ejob.busy; without this, multiple thing() calls get past the busy
        # check before the first one sets ejob.busy = True
...
async def thing():
    try:
        ...
        if not queue:
            ...
            return
        # If a job is already running (ejob.busy), don't start another
        if ejob.busy:
            await asyncio.sleep(1)
            return
        while True:
            ...
            if len(queue) == queue_no:
                break
            await asyncio.sleep(1)
        # ponytail: claim the slot before any await so the loop can't start a second job
        ejob.busy = True
        queue_id = list(queue.keys())[0]
        ...
        # Batches shenanigans
        batch_dl_path = None  # full path to the specific batch file to download
        if m[1].lower() == "batch.":
            einfo.batch = True
            batch_dl_path, index, name = get_downloadable_batch(queue_id)
            einfo.select = index
            n = None  # To prevent hell from breaking loose
            if name is None:
                einfo.batch = None
                ejob.complete()
                await release_batch_torrent(einfo.uri)
                await skip(queue_id)
                await save2db()
        ...
        if chat_id == log_channel:
            ...
            op = None
        try:
            # For batch: use the full path from get_downloadable_batch (file inside torrent folder)
            # For normal: reconstruct from name as before
            dl = batch_dl_path if einfo.batch else "downloads/" + name
            if einfo.uri:
                if m[0] == "aria2" and m[1].split()[0].lower() == "select.":
            ...
            downloaded = await download.start(
                ...
                target_path=batch_dl_path if einfo.batch else None
            )
            ...
            if not downloaded or download.is_cancelled:
                ejob.complete()
                # ponytail: a single download failure inside a batch (aria2 RPC
                # hiccup, timeout, transient network error, etc.) used to mark
                # that episode "done" immediately, permanently dropping it from
                # the batch with no retry — this is what showed up as the bot
                # "skipping" episodes. Give a genuinely-failing episode a couple
                # of retries before giving up on it for real. A user-cancelled
                # download is intentional, so that still skips straight through.
                if (
                    einfo.batch
                    and not download.is_cancelled
                    and batch_dl_should_retry(einfo.select, queue_id)
                ):
                    await skip(queue_id)
                    await save2db()
                    await save2db("batches")
            ...
            if einfo.batch:
                clear_batch_dl_retry(einfo.select, queue_id)
            # For batch: keep batch_dl_path (the actual file); download.path is the torrent folder.
            dl = batch_dl_path if (einfo.batch and batch_dl_path) else download.path
        except AlreadyDl:
        ...
        # Heavily-subbed releases can embed 30-50+ fonts as attachment
        # streams. Handing all of those to the primary encode pass (most
        # user templates use a blanket "-map 0") makes ffmpeg's stream/option
        # handling unreliable at that scale. Fonts aren't needed for the
        # transcode itself — the mux pass below re-attaches the originals —
        # so drop them from what the encoder actually reads.
        encode_dl = await strip_excess_attachments(dl)

        _set = time.time()
        einfo.current = file_name
        einfo._current = name
        cmd = ffmpeg.format(encode_dl, out)
        ...
        org_s = size_of(dl)
        out_s = size_of(out)
        pe = 100 - ((out_s / org_s) * 100) if org_s else 0.00
        per = str(f"{pe:.2f}") + "%"
        ...
        st_msg = await up.reply(
            ...
            f"{'Cached' if getattr(einfo, 'cached_dl', False) else 'Downloaded'} in `{dtime}`\n"
        ...
        if not ejob.pending():
            ...
            if download:
                if not (download.is_cancelled or download.download_error):
                    # ponytail: for batch, don't delete files mid-batch —
                    # remaining episodes are still in the torrent folder.
                    # Only clean up when the entire bqueue for this job is empty.
                    from bot.utils.batch_utils import get_bqueue
                    _bq = get_bqueue()
                    _is_batch_active = any(
                        bool(v[1])  # batch_q dict still has pending items
                        for v in _bq.values()
                    )
                    if not _is_batch_active:
