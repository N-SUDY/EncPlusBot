...
async def get_queue_msg():
    ...
    try:
        # Take a single snapshot of the dictionary items to prevent race conditions
        queue_items = list(_bot.queue.items())
        i = len(queue_items)
        global STATUS_START, PAGE_NO, PAGES
        PAGES = (i + STATUS_LIMIT - 2) // STATUS_LIMIT
        if PAGE_NO > PAGES and PAGES != 0:
            STATUS_START = (STATUS_LIMIT * PAGES) - 9
            PAGE_NO = PAGES

        # Iterate directly over the snapshot
        for (queue_key, file), _no in zip(
            queue_items[STATUS_START : STATUS_LIMIT + STATUS_START],
            itertools.count(STATUS_START),
        ):
            chat_id, msg_id = queue_key
