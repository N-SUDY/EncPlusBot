...
# ponytail: previously, any download failure for an episode inside a batch
# (aria2 error, RPC hiccup, timeout, etc.) caused that episode to be marked
# "done" (status 2) on the very first failure — permanently removing it from
# the batch with no retry. From the user's perspective this looked like the
# bot randomly "skipping" an episode partway through a batch. We now give
# each episode a small retry budget before giving up on it for real.
_BATCH_DL_RETRIES = {}
MAX_BATCH_DL_RETRIES = 2  # total attempts = 1 + this


def batch_dl_should_retry(file_id, q_id):
    """Track per-episode download-failure retries within a batch.

    Returns True if this episode's status should be left as pending so it
    gets picked up again on the next pass, False if it has exhausted its
    retry budget and should now be marked done/skipped for real.
    """
    if file_id is None:
        return False
    key = (q_id, file_id)
    count = _BATCH_DL_RETRIES.get(key, 0)
    if count < MAX_BATCH_DL_RETRIES:
        _BATCH_DL_RETRIES[key] = count + 1
        return True
    _BATCH_DL_RETRIES.pop(key, None)
    return False


def clear_batch_dl_retry(file_id, q_id):
    _BATCH_DL_RETRIES.pop((q_id, file_id), None)
...
async def batch_preview(
    ...
    try:
        ...
        batch_queue = get_bqueue()
        if reuse:
            # ponytail: an edit session can stay open for a while (it polls
            # every ~5s waiting on the user), and the auto-worker keeps
            # processing the batch live the whole time. `preview_queue` is a
            # snapshot taken when the edit started, so blindly overwriting
            # the live entry with it would revert any episode the worker
            # finished (status 2) in the meantime back to "pending" — the
            # worker would then find it still marked pending and reprocess
            # it, effectively knocking the *next* real episode out of turn.
            # Re-apply "done" from the current live state on top of the
            # user's edits so in-flight progress is never lost.
            _live = batch_queue.get((chat_id, e_id))
            if _live:
                for _k, _v in _live[1].items():
                    if _v == 2:
                        pre_queue[_k] = 2
...
def mark_file_as_done(file_id, q_id):
    ...
    clear_batch_dl_retry(file_id, q_id)
