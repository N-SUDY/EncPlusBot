...
# Python 3.10+ no longer creates an implicit event loop — create one explicitly
# before pyrogram's sync wrapper tries to call asyncio.get_event_loop().
try:
...
from .utils.bot_utils import get_var
...
from .workers.handlers.e_callbacks import (
    dl_stat,
...
try:
    ...
    LOGS.info(er)
    LOGS.critical(
        "Failed to start Telegram clients. Ensure APP_ID, API_HASH, BOT_TOKEN, and OWNER are set correctly."
    )
    sys.exit(1)
...
@tele.on(events.NewMessage(pattern=command(["autoencode"])))
async def _(e):
    await event_handler(e, allowautoencode)


@tele.on(events.NewMessage(pattern=command(["autothumb"])))
async def _(e):
    await event_handler(e, allowautothumb)


@tele.on(events.NewMessage(pattern=command(["delthumb"])))
async def _(e):
    await event_handler(e, del_thumb)
...
async def _(e):
    await download_button_callback(e)
...
@tele.on(events.NewMessage(pattern=command(["lb"])))
async def _(e):
    await event_handler(e, enlb, pyro)
...
# AI COMMANDS REMOVED (TheMisfitDK fork)
...
# Auto-encode on file receive — toggle with /autoencode on|off (seeded from AUTO_ENCODE env var)
@pyro.on_message(filters.incoming & (filters.video | filters.document))
async def _(pyro, message):
    if get_var("autoencode"):
