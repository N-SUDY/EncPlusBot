...
RUN arch=$(arch | sed s/aarch64/arm64/ | sed s/x86_64/64/) && \
    wget -q https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-n8.1-latest-linux${arch}-gpl-8.1.tar.xz && tar -xvf *xz && cp *8.1/bin/* /usr/bin && rm -rf *xz && rm -rf *8.1
